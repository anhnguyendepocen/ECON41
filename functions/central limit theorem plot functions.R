...
clt_plot_pois <- function(n, trials, lambda){
  ...
  pdf <- dat %>% ggplot(aes(x, prob)) + geom_line(size = 1) + geom_vline(xintercept = lambda, size = 1, linetype = 'dashed', color = 'red') + scale_y_continuous(limits = c(0, max(dat$prob) + 0.05)) + labs(title = 'PDF') + ds_theme_set()
