...
as.data.frame(scale(cfsb)) %>% ggplot(aes(weight)) + geom_histogram(aes(y = stat(count / sum(count))), color = 'black', fill = 'red', bins = 7) + stat_function(fun = dnorm, args = list(mean = 0, sd = 1), size = 1, color = 'blue') + labs(y = 'Proportion', x = 'Number of standard deviations from mean') + scale_x_continuous(limits = c(-3, 3))